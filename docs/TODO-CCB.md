...
Ideas worth adopting in jackin, prioritized by effort/impact.

## P1: Req-ID protocol for pane communication

CCB wraps every message with `CCB_REQ_ID: <id>` and requires `CCB_DONE: <id>` to
close it. Multiple concurrent requests to the same pane never mix up. Jack-In
...
calls on every watchdog tick. Jack-In currently snapshots panes per tick —
...
`plan_log.md`) that survive context window resets. Jack-In tasks are individual
...
requests. The jackin orchestrator could benefit from similar guardrails when
...
a scoring framework (pass if overall >= 7.0 and all dimensions >= 3.0). Jack-In
...
context from a completed task to the next related one. Jack-In workers start
...
jackin ever wants Codex/Gemini workers alongside Claude, an adapter pattern
