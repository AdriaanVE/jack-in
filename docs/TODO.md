...
- [x] Event-driven orchestrator automation -- send message to Claude when task
      finishes (or other events) instead of relying on polling/manual checks
      (implemented in orchestrator_notify.go: JACKIN_EVENT messages sent via tmux)
- [ ] Worker completion comment -- workers should provide a comment when marking
      a task complete (what was done, any caveats). Store as `completionNote` in
      task JSON, visible in: `jackin tasks` CLI, TUI review panel, orchestrator
      event notification (TASK_REVIEW), and `jackin status --json`.
...
- [ ] Make evaluator model configurable in `jack-in.yaml` (currently hardcoded
...
- [ ] `deno install` -- creates wrapper script in `~/.deno/bin/jackin`,
...
- [ ] JSR package -- publish to Deno registry, install via
      `deno install jsr:@jackin/jackin`
...
- [ ] GitHub Actions CI: run `go build`, `go test`, `go vet` on PRs
- [ ] CONTRIBUTING.md: contribution guidelines, development setup, PR process
- [ ] goreleaser config for automated binary builds on release tags
